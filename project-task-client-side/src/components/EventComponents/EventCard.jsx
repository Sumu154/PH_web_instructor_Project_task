...
const EventCard = ( { event } ) => {
  const {  _id:id, title, image, regStart, regEnd, eventStart, location, distance, description, regCount } = event;
  ...
  return (
    <div className='border-black dark:border-[#444444] border-[1px] border-opacity-15 rounded-md p-3 hover:border-magenta hover:border-opacity-50 hover:p-2  dark:bg-cardbackground '>
      ...
      <div className='mt-3 '> <Link to={`/events/${id}`}> <button className='bg-magenta opacity-90 hover:opacity-100 px-3 py-1 rounded-md text-white '> Details </button> </Link> </div>
    </div>
  );
